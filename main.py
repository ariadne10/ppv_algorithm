...
if review_file:
    ...
    comparison_cols = ['Contact Email', 'BU Name', 'Offer JPN', 'Offer MPN']
    merged_data.set_index(comparison_cols, inplace=True)
    review_df.set_index(comparison_cols, inplace=True)
    
    # Find the rows that are not present in merged_data
    diff_df = review_df.loc[~review_df.index.isin(merged_data.index)]
    
    # Append the diff_df to merged_data
    merged_data = pd.concat([merged_data.reset_index(), diff_df.reset_index()])
